...
def cast_list(title)
  ...
  Actor.select(:id, :name).joins(:movies).where("title = ?", title).order("ord")
...
def vanity_projects
  ...
  # Note: Directors appear in th e 'actors' table.
  Movie.select(:id, :title, :name).joins(:director).joins(:castings).where("director_id = actor_id AND ord = 1")
